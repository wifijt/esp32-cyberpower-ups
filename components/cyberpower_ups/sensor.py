...
from esphome.const import UNIT_WATT, UNIT_VOLT_AMPS, UNIT_PERCENT, DEVICE_CLASS_BATTERY, DEVICE_CLASS_POWER, ICON_TIMER
...
CONFIG_SCHEMA = cv.Schema({
    ...
    cv.Optional(CONF_RUNTIME): sensor.sensor_schema(unit_of_measurement="min", icon=ICON_TIMER, accuracy_decimals=0), # Added
})
...
async def to_code(config):
    ...
    if CONF_RUNTIME in config: # Added
        sens = await sensor.new_sensor(config[CONF_RUNTIME])
        cg.add(hub.set_runtime_sensor(sens))
